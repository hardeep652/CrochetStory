...
const GiftArticles: React.FC = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const url = `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/products/getproducts/gift-articles`;
  ...
  const handleBuyNow = (product: Product) => {
    router.push(`/enquiry?category=${encodeURIComponent("Gift Articles")}&product=${encodeURIComponent(product.product_name)}&price=${encodeURIComponent(product.price)}&description=${encodeURIComponent(product.description)}`);
  };
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          >
            <div className="inline-flex items-center space-x-2 bg-gradient-to-r from-pink-100/80 to-rose-100/80 backdrop-blur-sm px-4 py-2 rounded-full border border-pink-200/50">
              ...
              <span className="text-sm font-medium text-pink-700">Gift Articles Collection</span>
            </div>
          </motion.div>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            <h2 className="text-3xl lg:text-5xl font-bold bg-gradient-to-r from-gray-900 to-pink-700 bg-clip-text text-transparent">
              Our Gift Articles Collection
            </h2>
            <p className="text-xl text-gray-600 max-w-2xl mx-auto">
              Browse our curated selection of handcrafted crochet gift articles.
            </p>
          </motion.div>
                        ...
                        {Array.isArray(product.imageURLs) && product.imageURLs.length > 0 ? (
                          product.imageURLs.length === 1 ? (
                            <img
                              ...
                              alt={`${product.product_name || "Gift Articles crochet product"}`}
                              ...
                              {product.imageURLs.map((url, idx) => (
                                <div key={idx} className="w-full h-[200px]" onClick={() => handleImageClick(url || "/placeholder.jpg")}>
                                  <img
                                    ...
                                    alt={`${product.product_name || "Gift Articles crochet product"} - Image ${idx + 1}`}
                                    className="w-full h-full object-cover cursor-zoom-in hover:scale-[1.02] transition-transform duration-300"
                                    onError={(e) => { e.currentTarget.src = "/placeholder.jpg"; }}
                                  />
                                </div>
                          ...
                          <img
                            src="/placeholder.jpg"
                            alt="Gift Articles crochet placeholder"
                            className="w-full h-full object-cover cursor-zoom-in hover:scale-[1.02] transition-transform duration-300"
