...
const Others: React.FC = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const url = `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/products/getproducts/others`;
  ...
  const handleBuyNow = (product: Product) => {
    router.push(`/enquiry?category=${encodeURIComponent("Others")}&product=${encodeURIComponent(product.product_name)}&price=${encodeURIComponent(product.price)}&description=${encodeURIComponent(product.description)}`);
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-white via-pink-50 to-rose-100 relative overflow-x-hidden">
      ...
      <section className="relative overflow-hidden py-10 lg:py-10">
        <div className="absolute inset-0 overflow-hidden">
          ...
          ></motion.div>
          <motion.div
            className="absolute bottom-20 left-1/4 w-16 h-16 bg-pink-300/80 rounded-full blur-lg"
            animate={{ x: [-5, 5, -5], opacity: [0.2, 0.4] }}
            transition={{ repeat: 2, duration: 5 }}
          />
          <div className="absolute top-1/4 left-1/3 w-24 h-24 border-2 border-pink-200/40 rounded-full animate-pulse opacity-40"></div>
          ...
          <div className="absolute inset-0 opacity-[0.03]">
            ...
            <div className="absolute top-4 left-32 w-1 h-1 bg-rose-400 rounded-full"></div>
            <div className="absolute top-24 right-2 w-2 h-2 bg-pink-500 rounded-full"></div>
            <div className="absolute top-48 right-48 w-1 h-1 bg-rose-400 rounded-full"></div>
            <div className="absolute bottom-32 left-2 w-2 h-2 bg-pink-400 rounded-full"></div>
            <div className="absolute bottom-48 right-1 w-1 h-1 bg-rose-400 rounded-full"></div>
          </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="text-center space-y-2">
            ...
            >
              ...
              <span className="text-sm font-medium text-pink-700">Others Collection</span>
            </motion.div>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            ...
            <p className="text-xl text-gray-600 max-w-2xl mx-auto">
              Browse our unique handcrafted crochet items.
            </p>
          </motion.div>
                        ...
                        {Array.isArray(product.imageURLs) && product.imageURLs.length > 0 ? (
                          product.imageURLs.length === 1 ? (
                            <img
                              ...
                              alt={`${product.product_name || "Others crochet product"}`}
                              ...
                              {product.imageURLs.map((url, idx) => (
                                <div key={idx} className="w-full h-[200px]" onClick={() => handleImageClick(url || "/placeholder.jpg")}>
                                  <img
                                    ...
                                    alt={`${product.product_name || "Others crochet product"} - Image ${idx + 1}`}
                                    className="w-full h-full object-cover cursor-zoom-in hover:scale-[1.02] transition-transform duration-300"
                                    onError={(e) => { e.currentTarget.src = "/placeholder.jpg"; }}
                                  />
                                </div>
                          ...
                          <img
                            src="/placeholder.jpg"
                            alt="Others crochet placeholder"
                            className="w-full h-full object-cover cursor-zoom-in hover:scale-[1.02] transition-transform duration-300"
