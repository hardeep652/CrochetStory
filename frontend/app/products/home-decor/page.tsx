...
const HomeDecor: React.FC = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const url = `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/products/getproducts/home-decor`;
  ...
  const handleBuyNow = (product: Product) => {
    router.push(`/enquiry?category=${encodeURIComponent("Home Decor")}&product=${encodeURIComponent(product.product_name)}&price=${encodeURIComponent(product.price)}&description=${encodeURIComponent(product.description)}`);
  };
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          >
            <div className="inline-flex items-center space-x-2 bg-gradient-to-r from-pink-100/80 to-rose-100/80 backdrop-blur-sm px-4 py-2 rounded-full border border-pink-200/50">
              ...
              <span className="text-sm font-medium text-pink-700">Home Decor Collection</span>
            </div>
          </motion.div>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            ...
            <p className="text-xl text-gray-600 max-w-2xl mx-auto">
              Browse our curated selection of handcrafted crochet home decor.
            </p>
          </motion.div>
                        ...
                        {Array.isArray(product.imageURLs) && product.imageURLs.length > 0 ? (
                          product.imageURLs.length === 1 ? (
                            <img
                              ...
                              alt={`${product.product_name || "Home Decor crochet product"}`}
                              ...
                              {product.imageURLs.map((url, idx) => (
                                <div key={idx} className="w-full h-[200px]" onClick={() => handleImageClick(url || "/placeholder.jpg")}>
                                  <img
                                    ...
                                    alt={`${product.product_name || "Home Decor crochet product"} - Image ${idx + 1}`}
                                    className="w-full h-full object-cover cursor-zoom-in hover:scale-[1.02] transition-transform duration-300"
                                    onError={(e) => { e.currentTarget.src = "/placeholder.jpg"; }}
                                  />
                                </div>
                          ...
                          <img
                            src="/placeholder.jpg"
                            alt="Home Decor crochet placeholder"
                            className="w-full h-full object-cover cursor-zoom-in hover:scale-[1.02] transition-transform duration-300"
